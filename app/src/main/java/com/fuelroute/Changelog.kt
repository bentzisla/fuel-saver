package com.fuelroute

/**
 * In-app "מה חדש" (what's new) changelog. One [Entry] per released version; the newest entry is
 * shown first. The orchestrator appends an entry and bumps `versionCode`/`versionName` in
 * `app/build.gradle.kts` for every merged feature/bug-fix wave (see docs/IMPLEMENTATION-PLAN.md).
 */
object Changelog {

    data class Entry(
        val versionName: String,
        val title: String,
        val changes: List<String>,
    )

    val entries: List<Entry> = listOf(
        Entry(
            versionName = "0.7.3",
            title = "עלות מסלול מדויקת, עלות בפועל לכל נסיעה, OBD יציב יותר",
            changes = listOf(
                "תוקנה עלות מסלול נמוכה בעשרות מונים: מקדם כיול שגוי (למשל מנסיעת הדגמה) כבר לא מקטין את הצריכה, והוא מוגבל לטווח סביר",
                "הגנות מפני נתונים לא סבירים: עקומה נלמדת, צריכה מוצהרת ומחיר דלק",
                "העלייה נכללת בחישוב (למשל בית שמש–ירושלים) לפי משקל הרכב; ללא נתוני גובה מוצגת הערה",
                "בפרטי המסלול: צריכה ממוצעת, מקור העקומה ומקדם הכיול",
                "אפשר להזין או לערוך את העלות בפועל לכל נסיעה, גם בלי OBD; הגיבוי שומר אותה",
                "OBD: מנוע כבוי כבר לא גורם לחיבורים חוזרים בלולאה",
                "OBD: תקלה רגעית בנסיעה מטופלת בהדרגה (המתנה, סנכרון רך) לפני חיבור מחדש, עם זמן המתנה ארוך יותר לרכבים איטיים",
            ),
        ),
        Entry(
            versionName = "0.7.2",
            title = "ניווט חלק יותר במסלול שבחרת",
            changes = listOf(
                "המסלול המומלץ נפתח ב-Google Maps ישר לניווט, בלי תצוגה מקדימה ובלי עצירות",
                "מסלול חלופי: רק נקודת ביניים אחת או שתיים במקום ארבע, לפי אימות מול Google, במקום עצירות מרובות",
                "Waze מקבל יעד בלבד ולא מסלול: האפליקציה שואלת אם לנווט ב-Google Maps במסלול המדויק",
            ),
        ),
        Entry(
            versionName = "0.7.1",
            title = "הכנה ל-Google Play",
            changes = listOf(
                "גרסת Play נפרדת (ללא שכבת-על וללא בקשת פטור מסוללה); Android Auto נשאר בכל הגרסאות",
                "Android Auto: כותרת תואמת למארחים ישנים, כפתור החיבור לא מפעיל עוד נסיעת הדגמה, וכשל בהפעלת השירות לא מפיל את מסך הרכב",
                "כיווץ קוד (R8), קישור למדיניות פרטיות בהגדרות ואייקון מונוכרומטי",
            ),
        ),
        Entry(
            versionName = "0.7.0",
            title = "עיצוב חדש, חיבור OBD אמין ותיקון צריכת דלק שגויה",
            changes = listOf(
                "עיצוב מחודש ונקי לכל המסכים: תוצאה מומלצת אחת עם ניווט בלחיצה, פרטים מתקפלים",
                "תוקנה צריכה של מאות/אלפי ליטרים ל-100 ק\"מ: נפח מנוע שהוזן בסמ\"ק מתוקן אוטומטית לליטרים",
                "צריכה רגעית מוחלקת; במהירות נמוכה מוצגים ליטר/שעה במקום ל׳/100 ק\"מ",
                "נתוני הלמידה והנסיעות שנפגעו משוחזרים מהדגימות הגולמיות (עם גיבוי לפני שינוי)",
                "חיבור לדונגל OBD אמין יותר: אין עוד צורך לנתק ולחבר את הדונגל פיזית",
                "האפליקציה כבר לא נתקעת כשהדונגל מפסיק לענות",
                "איפוס/אימוץ עקומה ושחזור גיבוי כבר לא נדרסים בזמן הקלטה",
                "החיסכון בהיסטוריה מחושב לפי המסלול שבחרת בפועל",
            ),
        ),
        Entry(
            versionName = "0.6.4",
            title = "תיקון ניווט ותצוגה + שמירה על נתונים",
            changes = listOf(
                "תוקנה מסירת היעד לאפליקציית הניווט (Google Maps נפתח ישירות לניווט)",
                "תוקנה תצוגת כתובות היסטוריה/מועדפים שהוצגו בגודל מוזר",
                "הגנה מפני אובדן נתונים: שדרוג מסד הנתונים כבר לא מוחק נתונים בשתיקה",
            ),
        ),
        Entry(
            versionName = "0.6.3",
            title = "ביצועים ופרטיות",
            changes = listOf(
                "הוספת אינדקסים למסד הנתונים לשיפור ביצועים (שדרוג סכמה 8→9)",
                "מחיקת רכב מוחקת כעת גם את כל הנתונים הקשורים אליו",
                "הנתונים הרגישים (נסיעות, VIN, כתובות) אינם מגובים עוד לענן",
            ),
        ),
        Entry(
            versionName = "0.6.2",
            title = "ממשק והתחלה ראשונה",
            changes = listOf(
                "מסך התחלה ראשונה עם שלבי הגדרת מפתח Google ורכישת דונגל ELM327",
                "אישור לפני פעולות מחיקה ואיפוס, ולחצנים נוחים יותר להקשה",
                "תיקון תצוגת גרף בעברית (RTL) וצבעים קריאים יותר במצב כהה",
                "טיפול טוב יותר בשגיאות והסרת חסימה בשרשור הראשי",
            ),
        ),
        Entry(
            versionName = "0.6.1",
            title = "אמינות רשת ונתונים",
            changes = listOf(
                "חישוב חיובי אגרה שלא נדרסים על ידי נתוני אגרה ריקים",
                "פתרון שגיאת 403 בתרגום מיקום לכתובת",
                "ניסיונות חוזרים וזמני תפוגה לקריאות רשת, וטיפול ברור יותר בפסקי זמן",
                "שמירת מחיר לליטר וסוג דלק ברישומי תדלוק, וגיבוי בטוח ועקבי יותר",
            ),
        ),
        Entry(
            versionName = "0.6.0",
            title = "מודל דלק מדויק יותר",
            changes = listOf(
                "תיקון חישוב צריכה כשחסר זמן מקטע (מניעת הערכת־יתר של עלות הדלק)",
                "למדת התנעה קרה הופעלה ומשפיעה כעת על חיזוי העלות",
                "תיקון כיול כפול בחישוב מקדם התיקון",
                "שיפור חישוב עומס תנועה והגנות מערכים לא תקינים",
            ),
        ),
        Entry(
            versionName = "0.5.2",
            title = "יציבות חיבור OBD",
            changes = listOf(
                "סגירה נקייה של החיבור והנסיעה גם בעת עצירה ידנית או ניתוק (מניעת דליפת חיבור ואובדן נתונים)",
                "ביטול מרוץ בין התחלה לעצירה ברישום OBD",
                "תמיכה בדונגלים מסוג STN/OBDLink, זמן אתחול ארוך יותר ל־ATZ",
                "מניעת עצירה שגויה ברכבים שאינם מדווחים סל״ד, וכתיבת דגימות בקבוצות",
            ),
        ),
        Entry(
            versionName = "0.5.1",
            title = "תיקון מיזוג ופיצול נסיעות",
            changes = listOf(
                "תוקן מיזוג/פיצול נסיעות שלא פעל עבור נסיעות מקושרות (זמן חיפוש מול זמן נסיעה)",
                "משוב ברור בעת הצלחה או כישלון במיזוג/פיצול",
                "מיזוג מותר רק בין נסיעות מאותו סוג (אמיתי או הדגמה)",
            ),
        ),
        Entry(
            versionName = "0.5.0",
            title = "היסטוריית נסיעות משופרת",
            changes = listOf(
                "הזנת עלות נסיעה ידנית (מחיר ישיר או ק״מ + צריכה ממוצעת)",
                "מחיקה מרובה של נסיעות בהיסטוריה",
                "מיזוג נסיעות סמוכות לנסיעה אחת ופיצול נסיעה לפי זמן",
            ),
        ),
        Entry(
            versionName = "0.4.3",
            title = "Android Auto",
            changes = listOf(
                "תיקון זיהוי אפליקציית Android Auto ושיפור אימות מארח (release)",
                "אבחון עצמי מפורט בעת התחברות למערכת הרכב",
                "הבהרה: ברכב אמיתי נדרשת התקנה דרך חנות מהימנה; לבדיקה מקומית השתמשו ב־DHU עם גרסת debug",
            ),
        ),
        Entry(
            versionName = "0.4.2",
            title = "מכשירי OBD מועדפים",
            changes = listOf(
                "המכשיר שחובר לאחרונה מופיע בראש רשימת ההתחברות",
                "סימון מכשירי OBD כמועדפים (כוכב) — מועדפים מוצגים לפני השאר",
            ),
        ),
        Entry(
            versionName = "0.4.1",
            title = "עריכת עקומת הרכב",
            changes = listOf(
                "עריכת עקומת צריכה ידנית: הוספה/עריכה/מחיקת נקודות מהירות מול צריכה",
                "התחלה מעקומת ברירת מחדל, מהעקומה הידנית הנוכחית, או מעקומה ריקה",
            ),
        ),
        Entry(
            versionName = "0.4.0",
            title = "חיבור OBD אמין וניווט לפי המסלול הנבחר",
            changes = listOf(
                "תוקן ניתוק OBD מוקדם שנגרם מקריאת מתח סוללה שגויה",
                "חיבור מחדש אוטומטי אחרי ניתוק (כל עוד הדונגל מחובר)",
                "Google Maps עוקב כעת אחרי המסלול הנבחר (נקודות מעבר, ללא עצירות ביניים)",
                "הבהרה: Waze מקבל את היעד בלבד ובוחר מסלול בעצמו",
            ),
        ),
        Entry(
            versionName = "0.3.1",
            title = "יציבות ואיכות",
            changes = listOf(
                "שיפור אמינות חיבור OBD והתקדמות החיבור",
                "סימון נסיעות הדגמה, תיקוני היסטוריה ומחיקה",
                "היקף נסיעות לפי רכב, אגרות ו־Android Auto",
            ),
        ),
        Entry(
            versionName = "0.3.0",
            title = "גרף עקומת רכב",
            changes = listOf(
                "גרף אינטראקטיבי לעקומת הצריכה",
                "גרסאות ועמוד אודות",
                "תיקוני תצוגת מפה ומקלדת",
            ),
        ),
    )
}
